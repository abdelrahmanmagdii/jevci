...
Runner resources depend on repository visibility; each run records the actual CPU count and RAM. Each test container is limited to 2 CPUs. Prometheus has a 12 GiB memory limit; Caddy and Jaeger retain 6 GiB limits. Swap remains disabled. Prometheus requires a runner with RAM above its container limit; the observed public runner has about 16 GiB. Each profile has a 30-minute limit, including setup. The workflow has a 120-minute limit. A timeout, missing test result, failed test, or tracked source change cannot qualify a profile. Failures are not retried automatically. Version 2 of `run.json` records the selected profiles in `container_memory_limits_gib`, replacing the single shared memory-limit field.
